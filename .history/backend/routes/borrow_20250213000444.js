...
router.post("/:book_id", authMiddleware, async (req, res) => {
  try {
    ...
    const book = await pool.query("SELECT * FROM bookss WHERE id = $1", [req.params.book_id]);
    if (book.rows.length === 0) return res.status(404).json({ message: "ไม่พบหนังสือ" });

    await pool.query(
      "INSERT INTO borrowingss (user_id, book_id, borrow_date, return_date, status) VALUES ($1, $2, $3, $4, false)",
      [req.user.id, req.params.book_id, borrow_date, return_date]
    );

    await pool.query("UPDATE bookss SET available = false WHERE id = $1", [req.params.book_id]);
...
router.post("/return/:book_id", authMiddleware, async (req, res) => {
  try {
    const borrow = await pool.query(
      "SELECT * FROM borrowingss WHERE user_id = $1 AND book_id = $2 AND status = false",
      [req.user.id, req.params.book_id]
    );
    if (borrow.rows.length === 0) return res.status(404).json({ message: "ไม่มีประวัติการยืม" });

    await pool.query(
      "UPDATE borrowingss SET status = true WHERE user_id = $1 AND book_id = $2 AND status = false",
      [req.user.id, req.params.book_id]
    );

    await pool.query("UPDATE bookss SET available = true WHERE id = $1", [req.params.book_id]);
...
router.delete("/book/:book_id", authMiddleware, async (req, res) => {
  try {
    ...
    const book = await pool.query("SELECT * FROM bookss WHERE id = $1", [book_id]);
    ...
    const borrowCheck = await pool.query(
      "SELECT * FROM borrowingss WHERE book_id = $1 AND status = false",
      [book_id]
    );
