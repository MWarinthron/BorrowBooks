...
router.get("/my-books", authMiddleware, async (req, res) => {
  try {
    const borrowedBooks = await pool.query(
      "SELECT b.id, b.title, b.author, b.image, br.borrow_date, br.return_date FROM books b INNER JOIN borrowings br ON b.id = br.book_id WHERE br.user_id = $1 AND br.status = false",
      [req.user.id]
    );
